jQuery(document).ready(function($) {
  ...
  function SchedulePlan(element) {
    ...
    this.timelineUnitDuration = 5; //getScheduleTimestamp(this.timelineItems.eq(1).text()) - getScheduleTimestamp(this.timelineItems.eq(0).text());
  ...
  SchedulePlan.prototype.openModal = function(event) {
    ...
    this.modalBody.find('.event-info').html(modalContent);
  ...
  function InitTimeline() {
    var startTimes = $.map(window.talks, function(o) {
      return o['Start'];
    });
    var uniqueStartTimes = startTimes
  ...
  function InitTracks() {
    var eventRoot = $('#eventRoot');
    $.each(window.tracks, function(i, v) {
      var eventListItem = $('<ul id="track_' + (i + 1) + '"></ul>');
      var eventGroup = $('<li class="events-group"></li>');
      var eventGroupDescription = $('<div class="top-info"></div>');
      var eventGroupDescriptionLabel = $('<span>' + v + '</span>');
      var eventGroupList = $('<ul class="talks"></ul>');
      eventListItem.append(eventGroup);
      eventGroup.append(eventGroupDescription);
      eventGroupDescription.append(eventGroupDescriptionLabel);
      eventGroup.append(eventGroupList);
      eventRoot.append(eventListItem);
    });
  }

  function InitTalks() {
    var talkCount = 1;
    $.each(window.tracks, function(trackIndex, trackDescription) {
      var talksInTrack = window.talks.filter(function(a) {
        return a.Track === trackIndex + 1;
      });

      talksInTrack.sort(function(a, b) {
        return (
      ...
      var trackElement = $('#track_' + talkCount);
      var talkRoot = trackElement.find('.talks');

      $.each(talksInTrack, function(talkIndex, talk) {
        var x =
